...
Starting points, and never scripts. Each is written to build friction in, so that
the tool argues, flags uncertainty and exposes its own weak spots instead of producing
a confident wall of text. Adapt freely, replace the bracketed parts and remember the
[data decision aid](data_decision_aid.md) before you paste anything.

## Prompting for friction: five habits

1. Make it disagree with itself. Ask for the strongest case against its own answer.
2. Force uncertainty to the surface. 'Mark each claim as confident, uncertain or a
   guess, and say what would change your mind.'
...
> owner, deadline and priority: [PASTE REDACTED NOTES]. Only include owners and dates
> that are explicitly stated. Put anything vague in a separate list headed "needs an
> owner or unclear". Do not invent details.'
